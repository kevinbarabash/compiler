...
# or other keywords
body
  ->%let _ identifier _ %equal _ expr _ (%semi | %nl) _ body {% data =>
...
fact
  ->fact %lparen args %rparen {% data => ({tag: "App", func: data[0], args: data[2]}) %}
  | atom {% id %}
...
atom
  ...
  | %string {% data => {
      console.log(data);
      return {tag: "Lit", value: {tag: "LStr", value: data[0].text.slice(1, -1)}};
    } 
  %}
